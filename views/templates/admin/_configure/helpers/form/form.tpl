...
{block name="field"}
                ...
                <div class="translatable-field lang-{$language.id_lang}"
                    ...
                    <div class="col-lg-6">
                        {if isset($fields[0]['form']['images'])}
                            <img src="{$image_baseurl}{$fields[0]['form']['images'][$language.id_lang]}"
                                 class="img-thumbnail"/>
                        {/if}
    ...
    {/if}
    {if $input.type == 'textarea_b64'}
        <div class="row">
            <div class="col-lg-6">
                <textarea name="{$input.name}" id="{$input.name}"
                          rows="{count(explode("\n",base64_decode($fields_value[$input.name])))}"
                          style=" min-height: 120px; "
                          class="textarea-autosize">{base64_decode($fields_value[$input.name])}</textarea>
            </div>
        </div>
    {/if}
    {$smarty.block.parent}
