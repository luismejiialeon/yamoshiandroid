...
class AndroidSlider extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'androidslider_slides',
		'primary' => 'id_androidslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'androidslider` (`id_shop`, `id_androidslider_slides`)
	...
	public function delete()
	{
		...
		foreach ((array)$images as $image)
		{
			if (preg_match('/sample/', $image) === 0)
				if ($image && file_exists(dirname(dirname(__FILE__)).'/images/'.$image))
					$res &= @unlink(dirname(dirname(__FILE__)).'/images/'.$image);
		}

		$res &= $this->reOrderPositions();

		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'androidslider`
			WHERE `id_androidslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_androidslider_slides` = hs.`id_androidslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_androidslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'androidslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'androidslider` hs ON (hss.`id_androidslider_slides` = hs.`id_androidslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new AndroidSlider($row['id_slide']);
